			...
			<div className="mx-4 mt-4 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-start gap-3">
				...
				<div>
					...
					<p className="text-[11px] text-blue-600/80 dark:text-blue-400/80 mt-0.5">
						Cookies are stored in your browser&apos;s local state
						and are not persisted to the database. They will be
						cleared when you logout. Expired cookies are
						automatically removed on visit.
					</p>
				</div>
			</div>
			...
			<div className="flex items-center justify-between p-4 border-b border-border/50 shrink-0">
				<div className="flex items-center gap-3">
					...
					<div className="flex items-center gap-2">
						...
						{filterDomain &&
							filteredCookies.length !== cookies.length && (
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{filteredCookies.map((cookie, index) => (
							<CookieCard
								...
								onDelete={() =>
									handleDelete(cookie.domain, cookie.key)
								}
							/>
						))}
					</div>
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This action will remove all cookies for the current
							workspace from your local storage. This cannot be
							undone.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
