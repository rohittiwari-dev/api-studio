	...
	useEffect(() => {
		const fetchGlobalAuth = async () => {
			...
			if (activeWorkspace?.globalAuth) {
				const globalAuth =
					activeWorkspace.globalAuth as GlobalAuthState;
				if (globalAuth) {
			...
			<div className="space-y-3">
				<div className="grid gap-3 sm:grid-cols-2">
					<div>
						<FieldLabel
							required
							tooltip="The header or query parameter name"
						>
							Key
						</FieldLabel>
						<EnvironmentVariableInput
							...
							value={data.key || ''}
							onChange={(val) =>
								handleAuthDataChange({ ...data, key: val })
							}
							className="bg-muted/30 h-7 text-xs font-mono px-2"
						/>
						<FieldDescription>
							Header name or query param key
						</FieldDescription>
					</div>
					<div>
						...
						<EnvironmentVariableInput
							...
							value={data.value || ''}
							onChange={(val) =>
								handleAuthDataChange({ ...data, value: val })
							}
							className="bg-muted/30 h-7 text-xs font-mono px-2"
						/>
						<FieldDescription>
				...
				<div>
					<FieldLabel tooltip="Where to send the API key">
						Add to
					</FieldLabel>
			...
			<div className="space-y-4">
				...
				<div className="grid gap-3 sm:grid-cols-2">
					<div>
						<FieldLabel
							required
							tooltip="Username for Digest authentication"
						>
							Username
						</FieldLabel>
						<EnvironmentVariableInput
					...
					<div>
						<FieldLabel
							required
							tooltip="Password for Digest authentication"
						>
							Password
						</FieldLabel>
						<EnvironmentVariableInput
					...
					<div className="grid gap-3 sm:grid-cols-2">
						...
						<div>
							...
							<EnvironmentVariableInput
								value={data.qop || ''}
								onChange={(val) =>
									handleAuthDataChange({ ...data, qop: val })
								}
								className="bg-muted/30 h-7 text-xs font-mono px-2"
				...
				<div className="grid gap-3 sm:grid-cols-2">
					...
					<div>
						...
						<EnvironmentVariableInput
							value={data.token || ''}
							onChange={(val) =>
								handleAuthDataChange({ ...data, token: val })
							}
							className="bg-muted/30 h-7 text-xs font-mono px-2"
				...
				<div className="border-t border-border/40 pt-3">
					...
					<div className="grid gap-3 sm:grid-cols-2">
						<div>
							<FieldLabel
								required
								tooltip="Method used to sign the request"
							>
								Signature Method
							</FieldLabel>
							<Select
								...
								onValueChange={(val) =>
									handleAuthDataChange({
										...data,
										signatureMethod:
											val as TOauth1Auth['signatureMethod'],
									})
								}
							>
								<SelectTrigger className="h-7 text-xs font-mono bg-muted/30">
				...
				<div className="flex items-center gap-2">
					...
					<div>
						...
						<p className="text-[10px] text-muted-foreground">
							Apply authentication to all requests in this
							workspace
						</p>
					</div>
				</div>
				...
				<>
					...
					<div className="px-4 py-3 border-b border-border/40">
						...
						<Select
							value={authType}
							onValueChange={(val) =>
								handleAuthTypeChange(val as AuthType)
							}
						>
							...
							<p className="text-[10px] text-muted-foreground flex-1">
								{authType !== 'NONE' ? (
									<>
										...
										Global auth will be applied to requests
										using &quot;Inherit from Workspace&quot;
									</>
								) : (
									'No global authentication configured'
								)}
