...
const CodeSnippetPanel = () => {
	...
	}, [activeGroupId, activeGroup, activeClientKey]);
		...
		const generate = async () => {
			...
			try {
				...
				if (
					...
					const authData = activeRequest.auth.data as Record<
						string,
						string
					>;
					switch (activeRequest.auth.type) {
						...
						case 'API_KEY':
							if (authData.key && authData.value) {
								const k = substituteVariables(
									authData.key,
									envVariables,
								);
								const v = substituteVariables(
									authData.value,
									envVariables,
								);
				...
				const code = Array.isArray(codeArray)
					? codeArray[0]
					: codeArray;
				...
				<div className="flex items-center justify-between px-6 py-4 border-b border-border/40 bg-background/50 backdrop-blur-sm">
					<div className="flex items-center gap-4">
						...
						<Tabs
							...
							onValueChange={setActiveClientKey}
						>
							...
							<div className="flex flex-col items-center gap-4 p-8 rounded-2xl bg-muted/5 border border-border/20">
								...
								<div className="text-center">
									...
									<p className="text-xs text-muted-foreground mt-1">
										Enter a URL in your request to generate
										code
									</p>
								</div>
							</div>
