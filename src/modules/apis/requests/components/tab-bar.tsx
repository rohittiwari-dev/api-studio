...
const TabBar = () => {
	...
	const calculateVisibleTabs = useCallback(() => {
		if (!tabBarRef.current || isDragging) {
			return;
		}
		...
		const maxVisibleTabs = Math.max(
			1,
			Math.floor(availableWidth / tabWidth),
		);
		...
		<>
			...
			>
						...
						{visibleTabs.map((tab) => (
							...
							>
								<TabItem
									...
									onCloseOthers={() =>
										unsavedGuard.confirmCloseOthers(
											tab.id,
											() => closeOtherTabs(tab.id),
										)
									}
									onCloseAll={() =>
										unsavedGuard.confirmCloseAll(() =>
											closeAllTabs(),
										)
									}
									onTabClick={() => {
				...
				{hiddenTabs.length > 0 && (
					<Popover
						open={hiddenTabsOpen}
						onOpenChange={setHiddenTabsOpen}
					>
