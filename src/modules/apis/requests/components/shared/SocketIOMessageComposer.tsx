...
const SocketIOMessageComposer: React.FC<SocketIOMessageComposerProps> = ({
	...
	const handleArgFormatChange = (id: string, format: ArgFormat) => {
		onArgsChange(
			args.map((arg) => (arg.id === id ? { ...arg, format } : arg)),
		);
	};
	...
	const hasErrors = useMemo(() => {
		return args.some(
			(arg) =>
				arg.format === 'json' && getJsonError(arg.content, arg.format),
		);
	}, [args, getJsonError]);
	...
	return (
		<>
			<div
				className={cn('flex flex-col h-full min-h-0 gap-3', className)}
			>
				{/* Args List */}
