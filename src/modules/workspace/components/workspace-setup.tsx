...
const WorkspaceSetup = ({
	...
	const onSubmit = form.handleSubmit(async (data) => {
		...
		try {
			...
			if (result.error) {
				toast.error(
					'Failed to create Workspace: ' + result.error.message,
				);
				...
				<form className={cn('flex flex-col gap-4')} onSubmit={onSubmit}>
					<div className="grid gap-3">
						...
						<div className="space-y-1">
							<AddOnInput
								leftIcon={
									<Slash className="w-3 h-3 rotate-112" />
								}
								placeholder="my-workspace"
								disabled={isSubmitting}
			...
			>
				...
				<div className="flex flex-col items-center gap-6 mb-8">
					...
					<div className="text-center space-y-2">
						...
						<p className="text-sm text-muted-foreground">
							Set up your workspace to organize your API
							collections and collaborate with your team.
						</p>
					</div>
				</div>
				...
				<form className="flex flex-col gap-5" onSubmit={onSubmit}>
					...
					<p className="text-center text-xs text-muted-foreground/60">
						You can invite team members after creating your
						workspace
					</p>
				</form>
			</motion.div>
